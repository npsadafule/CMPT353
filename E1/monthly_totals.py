...
def pivot_months_pandas(data):
    ...
    grouped_monthly = data.groupby(["name","month"]).aggregate({"precipitation": "sum"}).reset_index()
    
    monthly = grouped_monthly.pivot(index = "name", columns = "month", values = "precipitation")
    
    grouped_counts = data.groupby(["name","month"]).aggregate({"precipitation": "count"}).reset_index() 
    
    counts = grouped_counts.pivot(index = "name", columns = "month", values = "precipitation")
